...
def get_players_and_action_space_list(g):
    if sum(g.agent_nums) != g.n_player:
        raise Exception(
            "agent number = %d 不正确，与n_player = %d 不匹配"
    ...
    for policy_i in range(len(g.obs_type)):
        ...
        action_space_list = [
            g.get_single_action_space(player_id) for player_id in players_id_list
        ]
...
def get_joint_action_eval(
    game, multi_part_agent_ids, policy_list, actions_spaces, all_observes
):
...
def run_game(
    g, env_name, multi_part_agent_ids, actions_spaces, policy_list, render_mode
):
    """
...
if __name__ == "__main__":
    ...
    run_game(
        game, env_type, multi_part_agent_ids, actions_space, policy_list, render_mode
    )
