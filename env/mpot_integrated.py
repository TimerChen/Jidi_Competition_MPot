...
class MPot_Integrated(Game, DictObservation):
    def __init__(self, conf):
        super(MPot_Integrated, self).__init__(
            n_player=-1,
        ...
        # XXX: reset will return state and reward, but in run_log.py,
        #  these guys use g.all_observes as state
        #  FUNNY! :(
        self.all_observes, _ = self.reset()
    ...
    def _state_filter(self, ts):
        ...
        for i, s in enumerate(ts.observation):
    ...
    def reset_map(self, init=False):
        ...
        print(
            f"Game: {self.game_name}, Senario:{scenario_id}, Player_num{self.n_player}"
        )
        print(
            f"Obs Space: {self.env.observation_spec()[0]}, Act Space:{self.env.action_spec()[0]}"
        )
    ...
    def last_game(self):
        ...
        return (
            self.current_episode == self.num_episode
            and self.current_scenario == len(self.scenario_pool) - 1
        )
    ...
    def step(self, joint_action):
        joint_action = self.decode(joint_action)
    ...
    def set_action_space(self):
        ...
        new_action_spaces = [
            [Discrete(space.num_values)] for space in origin_action_space
        ]
    ...
    def decode(self, joint_action):
        action_list = []
        if len(self.joint_action_space) == 1:
            action = joint_action[0][0].index(1)
            action_list.append(action)

        else:
            action1 = joint_action[0][0].index(1)
            action2 = joint_action[1][0].index(1)
